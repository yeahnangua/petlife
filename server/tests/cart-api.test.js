...
describe('cart apis', () => {
  ...
  it('merges quantity when adding the same product and spec key again', async () => {
    ...
    const addResponse = await request(app).post('/api/user/cart/items').send({
      product_id: 'p-001',
      spec_key: '3kg|鸡肉',
      spec_label: '3kg · 鸡肉',
      quantity: 2
    ...
    const cartResponse = await request(app).get('/api/user/cart')
  ...
  it('removes all invalid items in one operation', async () => {
    ...
    const deleteResponse = await request(app).delete('/api/user/cart/invalid-items')
  ...
  it('updates quantity and selected state for an existing cart item', async () => {
    ...
    const updateResponse = await request(app).put(`/api/user/cart/items/${item.id}`).send({
      quantity: 4,
      selected: false
    })
    ...
    const refreshedCart = await request(app).get('/api/user/cart')
